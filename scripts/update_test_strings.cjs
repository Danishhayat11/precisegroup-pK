...
files.forEach(f => {
  if (fs.existsSync(f)) {
    ...
    content = content.replace(/Cash \+ Asset Realized − Commission Paid/g, 'Cash/Bank + Asset Realized − Commission Paid');
    content = content.replace(/Cash plus Asset Realized minus Commission Paid/g, 'Cash/Bank plus Asset Realized minus Commission Paid');
