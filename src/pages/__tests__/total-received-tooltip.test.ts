...
const FORMULA = "Cash/Bank + Asset Realized \u2212 Commission Paid";
...
describe("Total Received tooltip", () => {
  ...
  it("registers the exact formula text in the KPI_FORMULAS map", () => {
    ...
    expect(source).toMatch(
      /received:\s*\{[^}]*short:\s*"Cash\/Bank \+ Asset Realized \u2212 Commission Paid"/,
    );
  });
  ...
  it("uses the U+2212 minus sign, not an ASCII hyphen", () => {
    ...
    expect(source).not.toMatch(/Cash\/Bank \+ Asset Realized - Commission Paid/);
  });
});
