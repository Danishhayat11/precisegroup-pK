...
const FORMULA = "Cash + Adjustment Realised \u2212 Commission Paid";
...
describe("Total Received tooltip", () => {
  ...
  it("registers the exact formula text in the KPI_FORMULAS map", () => {
    ...
    expect(source).toMatch(
      /received:\s*\{[^}]*short:\s*"Cash \+ Adjustment Realised \u2212 Commission Paid"/,
    );
  });
  ...
  it("uses the U+2212 minus sign, not an ASCII hyphen", () => {
    ...
    expect(source).not.toMatch(/Cash \+ Adjustment Realised - Commission Paid/);
  });
});
