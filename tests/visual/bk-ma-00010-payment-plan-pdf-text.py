...
BASE_DIR = ROOT / "__diffs__" / "bk-ma-00010-payment-plan-text"
...
def _update_latest_pointer() -> None:
    ...
    try:
        ...
        # Symlinks may be unsupported (e.g. Windows w/o privilege); the
        # per-run directory is what callers actually need.
        try:
            tmp.unlink()
        except OSError:
            pass
...
# Pinned real terms — must mirror src/lib/__tests__/bk-ma-00010.test.ts.
# (sr, particulars substring, due_date DD-MMM-YYYY, due_amount)
EXPECTED_TERMS = [
    (1,  "Down Payment",   "12-Apr-2025",  500_000),
    (2,  "Installment 01", "04-Apr-2025",  675_000),
    (3,  "Installment 02", "04-Jul-2025",  675_000),
    (4,  "Installment 03", "04-Oct-2025",  675_000),
    (5,  "Installment 04", "04-Jan-2026",  675_000),
    (6,  "Installment 05", "04-Apr-2026",  675_000),
    (7,  "Installment 06", "04-Jul-2026",  675_000),
    (8,  "Installment 07", "04-Oct-2026",  675_000),
    (9,  "Installment 08", "04-Jan-2027",  675_000),
    (10, "Possession",     "04-Oct-2027",  3_500_000),
]
    ...
    async with async_playwright() as pw:
        ...
        for attempt in range(1, ATTEMPTS + 1):
            ...
            try:
                ...
                if attempt == ATTEMPTS:
                    # Dump the rendered HTML so failures are diagnosable
                    # without re-running the test interactively.
...
async def main() -> int:
    ...
    for sr, label, date, amount in EXPECTED_TERMS:
        ...
        if label not in sched:
            failures.append(f"missing term {sr}: label {label!r} not found in schedule")
        if date not in sched:
            failures.append(f"missing term {sr} ({label}): due date {date!r} not found")
        if amt not in sched:
            failures.append(f"missing term {sr} ({label}): amount {amt!r} not found")
    ...
    def _diagnose(sr: int, label: str, date: str, amt: str) -> str:
        """Build a page-by-page match matrix + nearest snippet so the
        failure shows exactly which component is missing where."""
        header = f"  diagnostic for term {sr} ({label} / {date} / {amt}):"
        rows = [
            f"    {'page':<6}{'label':<8}{'date':<8}{'amount':<8}"
        ]
        for i, ptxt in enumerate(per_page, start=1):
            lab_hit = "OK" if label in ptxt else "MISS"
            date_hit = "OK" if date in ptxt else "MISS"
            amt_hit = "OK" if amt in ptxt else "MISS"
            rows.append(
                f"    {i:<6}{lab_hit:<8}{date_hit:<8}{amt_hit:<8}"
            )
        # Pull a small snippet from each page where any component matches
        # so the diff shows what *did* render in place of the missing piece.
        snips: list[str] = []
        for i, ptxt in enumerate(per_page, start=1):
            for needle in (label, date, amt):
    ...
    for sr, label, date, amount in EXPECTED_TERMS:
        ...
        found_pages = {
            ...
            if label in ptxt and date in ptxt and amt in ptxt
        }
    ...
    if failures:
        ...
        return 1
    print(
        f"OK: BK-MA-00010 Payment Plan PDF has {EXPECTED_PAGES} pages, "
